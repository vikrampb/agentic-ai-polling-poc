...
test.describe('AQA-2 – Boundary Conditions', () => {
  test('NON_US_PERSON user belonging to a named team is blocked from logging in', async ({ request }) => {
    const users = await getUsers(request);
    const blockedUser = users.find(u => u.export_status === 'NON_US_PERSON' && (u.team_name === 'PBE' || u.team_name === 'DPS'));

    if (!blockedUser) {
      const anyBlockedUser = users.find(u => u.export_status === 'NON_US_PERSON');
      expect(anyBlockedUser).toBeDefined();
      const response = await login(request, anyBlockedUser!.username, anyBlockedUser!.password);
  ...
  test('User with no team assigned but valid US_PERSON export status can still log in successfully', async ({ request }) => {
    const users = await getUsers(request);
  ...
  test('User with no team assigned and NON_US_PERSON export status is blocked from logging in', async ({ request }) => {
    const users = await getUsers(request);
...
test.describe('AQA-2 – Negative Tests', () => {
  test('User entering an incorrect password receives an invalid credentials error', async ({ request }) => {
    const users = await getUsers(request);
  ...
  test('Login attempt with valid username but empty password returns a missing credentials error', async ({ request }) => {
    const users = await getUsers(request);
    const anyUser = users[0];
    expect(anyUser).toBeDefined();
