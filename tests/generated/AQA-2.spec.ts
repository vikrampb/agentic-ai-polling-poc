...
test.describe('AQA-2 – Boundary Conditions', () => {
  test('user with null team_name and US_PERSON status can still log in', async ({ request }) => {
    const users = await getUsers(request);
...
test.describe('AQA-2 – Negative Tests', () => {
  test('NON_US_PERSON user is denied access regardless of team', async ({ request }) => {
    const users = await getUsers(request);
  ...
  test('login fails with incorrect password for a valid user', async ({ request }) => {
    ...
    const response = await login(request, anyUser!.username, 'InvalidPassword!99');
    expect(response.success).toBe(false);
