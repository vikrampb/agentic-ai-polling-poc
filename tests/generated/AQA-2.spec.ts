...
test.describe('AQA-2 – Happy Path', () => {
  test('PBE team user is successfully logged in and receives welcome message', async ({ request }) => {
  ...
  test('DPS team user is successfully logged in and receives welcome message', async ({ request }) => {
    const users = await getUsers(request);
...
test.describe('AQA-2 – Boundary Conditions', () => {
  ...
  test('User with no team assigned but valid US_PERSON status receives successful login response', async ({ request }) => {
    const users = await getUsers(request);
...
test.describe('AQA-2 – Negative Tests', () => {
  ...
  test('Login fails when username is provided but password field is left empty', async ({ request }) => {
    const users = await getUsers(request);
    const anyUser = users[0];
    expect(anyUser).toBeDefined();
