...
test.describe('AQA-2 – Happy Path', () => {
  test('PBE team user can log in successfully and receives welcome message', async ({ request }) => {
  ...
  test('DPS team user can log in successfully and receives welcome message', async ({ request }) => {
    const users = await getUsers(request);
...
test.describe('AQA-2 – Boundary Conditions', () => {
  ...
  test('All NON_US_PERSON users from any team are blocked from logging in', async ({ request }) => {
    const users = await getUsers(request);
  ...
  test('Login attempt with missing credentials returns an appropriate error message', async ({ request }) => {
...
test.describe('AQA-2 – Negative Tests', () => {
  test('User from PBE team cannot log in with an incorrect password', async ({ request }) => {
    const users = await getUsers(request);
  ...
  test('User from DPS team cannot log in with an incorrect password', async ({ request }) => {
    const users = await getUsers(request);
  ...
  test('Login attempt with a valid password but non-existent username is rejected', async ({ request }) => {
    const users = await getUsers(request);
    const anyUser = users[0];
    expect(anyUser).toBeDefined();

    const response = await login(request, 'nonexistentuser_xyz', anyUser.password);
    expect(response.success).toBe(false);
    expect(response.message).toBe('Invalid UserID/Password combination. Please verify.');
  });
});
