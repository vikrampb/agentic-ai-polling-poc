...
test.describe('AQA-1 – Happy Path', () => {
  ...
  test('US_PERSON user login response includes correct exportStatus', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
    const usUser = users.find(user => user.export_status === 'US_PERSON');
    expect(usUser).toBeDefined();
  ...
  test('All US_PERSON users in the system can log in successfully', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
...
test.describe('AQA-1 – Negative Tests', () => {
  ...
  test('Login attempt with a valid password but unknown username is rejected', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
    const usUser = users.find(user => user.export_status === 'US_PERSON');
    expect(usUser).toBeDefined();

    const response = await login(request, 'unknownuser_that_does_not_exist', usUser!.password);
