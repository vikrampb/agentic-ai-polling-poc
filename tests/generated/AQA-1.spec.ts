...
test.describe('AQA-1 – Boundary', () => {
  ...
  test('All NON_US_PERSON users are blocked from logging in with valid credentials', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
...
test.describe('AQA-1 – Negative', () => {
  test('US_PERSON user cannot log in with an incorrect password', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
    const usUser = users.find(user => user.export_status === 'US_PERSON');
    expect(usUser).toBeDefined();

    const response = await login(request, usUser!.username, 'incorrectPassword123!');
    expect(response.success).toBe(false);
  ...
  test('Login attempt with missing username is rejected', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
    const usUser = users.find(user => user.export_status === 'US_PERSON');
    expect(usUser).toBeDefined();

    const response = await login(request, '', usUser!.password);
    expect(response.success).toBe(false);
  });
});
