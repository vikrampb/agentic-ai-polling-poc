...
test.describe('AQA-1 – Boundary Conditions', () => {
  test('NON_US_PERSON user is blocked with correct error message', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
  ...
  test('US_PERSON user with team_name null can still log in successfully', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
    const usUserNoTeam = users.find(u => u.export_status === 'US_PERSON' && u.team_name === null);
    if (!usUserNoTeam) {
      test.skip();
      return;
    }
