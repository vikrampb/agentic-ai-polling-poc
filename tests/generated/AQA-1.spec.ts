...
test.describe('AQA-1 – Boundary Conditions', () => {
  test('NON_US_PERSON user is blocked and success flag is false', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
  ...
  test('user with null team_name who is US_PERSON can still log in', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
    const usUserNoTeam = users.find(user => user.export_status === 'US_PERSON' && user.team_name === null);
    ...
    const response = await login(request, usUserNoTeam.username, usUserNoTeam.password);
    expect(response.success).toBe(true);
    expect(response.message).toContain('Login successful');
  });

  test('every user in the system has a recognized export_status value', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
...
test.describe('AQA-1 – Negative Tests', () => {
  test('NON_US_PERSON user receives correct error message on login attempt', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
    const nonUsUser = users.find(user => user.export_status === 'NON_US_PERSON');
    expect(nonUsUser).toBeDefined();
  ...
  test('all NON_US_PERSON users are blocked with the correct error message', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
