...
test.describe('AQA-1 – Acceptance Criteria Tests', () => {
  test('If the export_status of the user says “US_PERSON”, the user is allowed to log in and should be pr…', async ({ request }) => {
  ...
  test('If the export_status of the user says “NON_US_PERSON”, the user is not allowed to log in and is i…', async ({ request }) => {
    const users = await getUsers(request);
  ...
  test('If an invalid password is used, regardless of export_status, the user instead presented with an e…', async ({ request }) => {
    const users = await getUsers(request);
    const user = users[0];
