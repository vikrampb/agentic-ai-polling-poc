...
test.describe('AQA-1 – Happy Path', () => {
  ...
  test('All US_PERSON users are able to log in successfully', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
...
test.describe('AQA-1 – Boundary Conditions', () => {
  test('NON_US_PERSON user is blocked from logging in and receives correct error message', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
  ...
  test('Login attempt with missing credentials returns missing credentials message', { tag: ['@regression'] }, async ({ request }) => {
...
test.describe('AQA-1 – Negative Tests', () => {
  test('US_PERSON user with incorrect password is denied login and receives invalid credentials message', { tag: ['@regression'] }, async ({ request }) => {
    ...
    const response = await login(request, usUser!.username, 'incorrect_password_xyz');
  ...
  test('US_PERSON user with incorrect username is denied login and receives invalid credentials message', { tag: ['@regression'] }, async ({ request }) => {
    ...
    const response = await login(request, 'invalid_username_xyz', usUser!.password);

    expect(response.success).toBe(false);
    expect(response.message).toBe('Invalid UserID/Password combination. Please verify.');
  });

  test('NON_US_PERSON user with incorrect password is still blocked with the export status error rather than credentials error', { tag: ['@regression'] }, async ({ request }) => {
    const users = await getUsers(request);
    const nonUsUser = users.find(user => user.export_status === 'NON_US_PERSON');
    expect(nonUsUser).toBeDefined();

    const response = await login(request, nonUsUser!.username, 'incorrect_password_xyz');
