...
test.describe('AQA-1 – Boundary Conditions', () => {
  ...
  test('first NON_US_PERSON user in the list is blocked from logging in', async ({ request }) => {
    const users = await getUsers(request);
...
test.describe('AQA-1 – Negative Tests', () => {
  test('NON_US_PERSON user is blocked from logging in and receives error message', async ({ request }) => {
    const users = await getUsers(request);
  ...
  test('NON_US_PERSON login response does not indicate success', async ({ request }) => {
    const users = await getUsers(request);
    const nonUsUser = users.find(user => user.export_status === 'NON_US_PERSON');
    expect(nonUsUser).toBeDefined();
